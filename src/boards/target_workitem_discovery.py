...
from src.boards.target_workitem_db import db_post_workitem
...
def main():
    try:
        ...
        query = text("SELECT target_organization_url, target_project_name, target_pat FROM devops_to_ados.db_devops_ado_migration_details")
        migration_details = db_session.execute(query).fetchall()
        # Process each row
        for row in migration_details:
            target_organization_url, target_project_name, target_pat = row
            org_name=extract_name(target_organization_url)
            if not target_organization_url or not target_project_name or not target_pat:
                logger.warning("Missing essential fields. Skipping...")
                continue
            logger.info(f"Processing project {target_project_name} on server {org_name}")
            print("------------------------------------------------------------------------------------------")
            print(f"Processing project {target_project_name} in organization {org_name}")
            print("------------------------------------------------------------------------------------------")
            # Discover work items
            work_item_ids = get_work_items_query(target_organization_url, target_project_name, target_pat, api_version='6.0')
            if work_item_ids:
                work_item_details_list = []
                work_items = get_work_item_details(target_organization_url, target_project_name, work_item_ids, target_pat, api_version='6.0')
                for work_item in work_items:
                    comments = get_work_item_comments(target_organization_url, target_project_name, work_item['id'], target_pat, api_version='6.0-preview.3')
                    work_item_details = extract_work_item_info(target_organization_url, target_project_name, work_item, comments)
                    work_item_details_list.append(work_item_details)
                save_work_items_to_db(work_item_details_list)
            else:
                logger.warning(f"No work items found for project {target_project_name}")
    except Exception as e:

...
# # Convert target_df to a dictionary for faster lookup
# target_dict = {
#     (row.project_name, row.pipeline_name): row
#     for row in target_df
# }

# # Perform the comparison and include all source rows
# for source_row in source_df:
#     key = (source_row.project_name, source_row.pipeline_name)
#     target_row = target_dict.get(key, None)  # Check if the source row exists in target
...
#             if source_value != target_value:
#                 mismatch_found = True
#                 break  # Break comparison if a mismatch is found

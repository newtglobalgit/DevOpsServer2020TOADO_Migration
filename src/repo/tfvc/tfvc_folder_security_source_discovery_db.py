...
from src.models.tfvc_folder_security_source_discovery_model import TFVCFolderSecuritySourceDetails


def db_post_tfvc_folder_security(data):
    db = None  
    try:
        ...
        # Create a new record
        new_record = TFVCFolderSecuritySourceDetails(
           collection_name = collection_name,
            project_name =project_name,
            tfvc_name = tfvc_name ,
            tfvc_branch_name =tfvc_branch_name,
        ...
        success_message = "Record created successfully for the tfvc folder security."
...
def db_get_tfvc_folder_security():
    db = None
    try:
        with SessionLocal() as db:
            
            records = db.query(TFVCFolderSecuritySourceDetails).all()

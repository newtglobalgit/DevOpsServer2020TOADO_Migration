...
class TFVCSecuritySourceDetails(Base):
    __tablename__ = 'db_devops_discovery_tfvc_security'  # Match the SQL table name
    __table_args__ = {'schema': 'devops_to_ados'} 
    
    tfvc_security_id = Column(Integer, primary_key=True, index=True)
    ...
    def to_dict(self):
        return {
           "tfvc_security_id" : self.tfvc_security_id, 
